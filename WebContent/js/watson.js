        ...
        window.BUFFERSIZE = 8192, $(document).ready(function() {
            ...
            tokenGenerator.getToken(function(err, token) {
                /*window.onbeforeunload = function() {
                    localStorage.clear()
                }, APP.token || (console.error("No authorization token available"), console.error("Attempting to reconnect..."), showError(err && err.code ? "Server error " + err.code + ": " + err.error : "Server error " + err.code + ": please refresh your browser and try again"));                
                */
                var viewContext = {
            ...
            exports.onFileProgress = function(options, ondata, running, onerror, onend, samplingRate) {
                ...
                return {
                    getToken: function(callback) {
                        xhrGet("api/watson", function(responseText){
                                APP.token = responseText;
                                callback(null, APP.token);
                        }, function(err){
                                console.log(err);
                        });
                    },
